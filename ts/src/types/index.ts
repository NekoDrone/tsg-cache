...
export type DB1Type = PageObjectResponse & {
	properties: {
		...
		tags: { id: string; type: 'multi_select'; multi_select: { id: string; name: string }[] };
...
export type DB3Type = PageObjectResponse & {
	properties: {
		name: { title: RichTextItemResponse[] };
		category: { id: string; type: 'select'; select: { id: string; name: string } };
...
export type DB4Type = PageObjectResponse & {
	properties: {
		type: { id: string; type: 'select'; select: { id: string; name: string } };
		name: { title: RichTextItemResponse[] };
	};
};
