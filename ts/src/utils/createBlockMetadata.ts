    ...
    if (notionBlockObj.has_children) {
        ...
        const otherProperties = {
            table:
                ...
                notionBlockObj.type == "table" &&
                notionBlockObj.table,
            ...
            columnWidth:
                type == BlockType.COLUMN && getColumnWidth(notionBlockObj),
            calloutIcon:
                type == BlockType.CB_2 &&
    ...
    const otherProperties = {
        ...
        image: {
            src:
                ...
                notionBlockObj.type == "image" &&
                getImageSrc(notionBlockObj),
            alt: "placeholder image desc. TODO: implement alt-text handler",
            caption: richTextToHtml(
                BlockType.IMAGE && notionBlockObj.type == "image"
                    ? notionBlockObj.image.caption
                    : [],
            ),
        },
        video: {
            src:
                ...
                notionBlockObj.type == "video" &&
                getVideoSrc(notionBlockObj),
...
const determineBlockType = (notionBlockObj: BlockObjectResponse): BlockType => {
    if (
        notionBlockObj.type == "heading_1" &&
        notionBlockObj.heading_1.is_toggleable
    )
    ...
    if (
        notionBlockObj.type == "heading_3" &&
        notionBlockObj.heading_3.is_toggleable
    )
    ...
    if (
        notionBlockObj.type == "child_database" &&
        notionBlockObj.child_database.title == "Events"
    )
        return BlockType.DB_1;
    if (
        notionBlockObj.type == "child_database" &&
        notionBlockObj.child_database.title == "Blog"
    )
        return BlockType.DB_2;
    if (
        notionBlockObj.type == "child_database" &&
        notionBlockObj.child_database.title == "Employers"
    )
        return BlockType.DB_3;
    if (
        notionBlockObj.type == "child_database" &&
        notionBlockObj.child_database.title == "FAQ"
    )
        return BlockType.DB_4;
    if (
        notionBlockObj.type == "child_database" &&
        notionBlockObj.child_database.title == "Volunteer"
    )
        return BlockType.DB_5;
    if (
        notionBlockObj.type == "child_database" &&
        notionBlockObj.child_database.title == "Links"
    )
