	...
	try {
		return richText
			.map((text) => {
				...
				if (text.annotations.bold) content = sanitiseHtml(`<strong>${content}</strong>`);
				if (text.annotations.italic) content = sanitiseHtml(`<em>${content}</em>`);
				if (text.annotations.strikethrough) content = sanitiseHtml(`<s>${content}</s>`);
				if (text.annotations.underline) content = sanitiseHtml(`<u>${content}</u>`);
				if (text.annotations.code) content = sanitiseHtml(`<code>${content}</code>`);
				if (text.href)
					content =
